...
chef_platform_provision "prod" do
  ...
  chef_server_frontends 2
  platform_domain "mydomain.com"
  chef_server_hostname_stub
  manage_hostname_stub
  reporting_hostname_stub
end
...
chef_platform_provision "prod" do
  ...
  reporting_fqdn 
  chef_server_fqdn 
  analytics_version 1.1.6
  manage_version 1.21.0
  reporting_version 1.5.1
  with_analytics true
  with_manage true
  with_reporting true
end
...
chef_platform do
  ...
  def chef_server(&block)
    @chef_server ||= ChefServerType.new
    @chef_server.instance_eval(&block) if block
    @chef_server
  end
  chef_server do
    topology "tier"
  ...
  analytics do
    version :latest
    api_fqdn "analytics.chefplatform.local"
    configuration({
      "actions_consumer" => {
        "hipchat_api_token" => "4yZvEWO6gnVhjtl3F6aU12obylgNXzzqmklI5jP0",
        "hipchat_enabled" => [
