...
require_relative './platform_spec.rb'
  ...
  module Provisioner
    class ChefPlatformSpec < PlatformSpec
      ...
      def self.new_spec(policy_group, platform_data)
        use_platform_data = platform_data
      ...
      def self.get_ready(policy_group)
        chef_server = Cheffish.default_chef_server
        chef_api = Cheffish.chef_server_api(chef_server)
        begin
          platform_data = chef_api.get("/data/#{policy_group}/ready_chef_platform")
          platform_data.delete('id') if platform_data['id']
          get = ChefPlatformSpec.new(platform_data)
        rescue Net::HTTPServerException => e
          if e.response.code == '404'
      ...
      def get_data
        begin
          if self.platform_data
            # self.platform_data
            strip_hash_nil(self.platform_data)
          end
        rescue
        end
      end

      def get_data_no_nodes
        begin
          if self.platform_data
            _data_no_nodes = deep_hashify(self.platform_data.dup)
            data_no_nodes = strip_hash_nil(_data_no_nodes.dup)
            data_no_nodes.delete('nodes')
            data_no_nodes.to_hash
          end
        rescue
        end
      end
      ...
      def chef_server_data
        ...
        strip_hash_nil(self.platform_data['chef_server'])
      end
      ...
      def analytics_config
        strip_hash_nil(self.platform_data['analytics']['configuration']) rescue {}
      end
      ...
      def supermarket_config
        self.platform_data['supermarket']['configuration'].to_hash rescue {}
      end


      def supermarket_data
        self.platform_data['supermarket'].to_hash rescue {}
      end
      ##
      #
      # delivery Specs
      ...
      def save_to_chef_platform_ready_nodes_data_bag(action_handler)
        _self = self
        ...
        ChefMetal.inline_resource(action_handler) do
          chef_data_bag_item "ready_chef_platform" do
            action :create
            data_bag 'prod'
